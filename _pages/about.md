---
permalink: /
title: "👋🏻 Hi there, I'm Shweta Kumari"
excerpt: "About me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

<center>
<img src="/images/banner.png" alt="banner logo" style="height: 100%; width:100%; background-size: cover"/>
</center>
<br />
<p style='text-align: justify;'> 
👩🏻‍🎓 A driven and motivated graduate student at the University of Southern California, pursuing a Master of Science in Computer Science. With a solid foundation in the field, I am set to graduate in <b> May 2024 </b>. I completed my undergraduate studies in Computer Science and Engineering at Guru Gobind Singh Indraprastha University, achieving the highest rank in my class.
</p>

<p style='text-align: justify;'> 
👩🏻‍💻 With three years of professional experience as a software engineer, I specialize in developing backend applications, REST APIs, microservices, data pipelines, and internal developer tools. My expertise extends to architectural enhancements, cloud infrastructure design and implementation, and optimizing large-scale applications for seamless performance across distributed systems with robust network infrastructures. </p>

<p style='text-align: justify;'> 
🎯 In addition to technical proficiency, I possess comprehensive end-to-end project management skills. My strengths encompass requirement planning, design, development, testing, deployment, and maintenance. As a Squad lead, I've demonstrated proficiency in Agile task management for streamlined project execution. I also have a track record of conducting technical workshops and seminars, supported by strong communication abilities.
</p>

Technical Skills
------
<b> Programming Languages:</b> Go, Python, C++, Java, PHP, HTML, CSS, Javascript, Angular <br>
<b> Tools & Technologies:</b> Docker, RabbitMQ, Redis, Jenkins, Git, Datadog, Jira, Rancher <br>
<b> Databases:</b> MySQL, Cassandra, Postgres, MongoDB, SQLite <br>
<b> Cloud Platforms:</b> AWS (S3 & DynamoDB) <br>
<b> Operating Systems:</b> Linux, macOS, Windows <br>

Leadership & Activities
------
<span style='text-align: justify;'>
📚 I presented the <b>Wifi Data Leakage Detection</b> paper at the IETET conference and received the prestigious <b>Best Paper Award</b> among 150 teams.
</span>
<span style='text-align: justify;'>
🔖 As a Squad Leader, I provided leadership by conducting standups, facilitating technical discussions, and coordinating Ramp-ups for new team members. My focus remained on ensuring timely achievement of team deliverables.
</span>
<span style='text-align: justify;'>
🦸🏻‍♀️ I participated in the AnitaB.org Grace Hopper Celebration (GHC) in 2022 and will also be attending the 2023 event. Additionally, I took on the role of a mentor for Women In Engineering.
</span>
<span style='text-align: justify;'>
🎨 I organized and managed my college's cultural fest in my junior year, displaying my strong passion for art and craft. I also secured the 2nd place in a state-level drawing competition during high school, highlighting my talent in drawing.
</span>